...
- 客户模型分配（无筛选项 mapFilterValue）
- 验收：无 filter list、allowKeywordSearch=false 时 options.mapFilterValue 仍注入 filter.type；切换 Tab / 分配模型触发 reload 后，顶部 Alert 仍显示 filter.type（缺则为失败）。
- _BizUnit(@components/BizUnit),_mockPreset(@root/mockPreset),remoteLoader(@kne/remote-loader),antd(antd)
